...
__global__ void computeMean2D_backward(int P, const float3* means3D, 
	...
	if (idx >= P)
		return;
	float3 p_orig = {means3D[idx].x, means3D[idx].y, means3D[idx].z};
	float3 p_orig_d = {means3D[idx].x + dL_dmean3Ds[idx].x, means3D[idx].y + dL_dmean3Ds[idx].y, means3D[idx].z + dL_dmean3Ds[idx].z};
	// Bring points to screen space
	float3 p_view = transformPoint4x3(p_orig, viewmatrix);
	float3 p_view_d = transformPoint4x3(p_orig_d, viewmatrix);

	dL_dmean2Ds[idx].x = (p_view_d.x / p_view_d.z - p_view.x / p_view.z); 
	dL_dmean2Ds[idx].y = (p_view_d.y / p_view_d.z - p_view.y / p_view.z); 
