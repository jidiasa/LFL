...
class GaussSplatRefiner:
    ...
    def create(self, camerapath, pts_coord_world, pts_colors, example_name=None, traindata_path=None, generate=True):

        outfile = os.path.join('examples', f'{example_name}.ply')
        if generate == True:
            self.traindata = self.generate_pcd(camerapath, pts_coord_world, pts_colors)
        else:
            self.traindata = load_traindata(traindata_path)
        self.scene = Scene(self.traindata, self.gaussians, self.opt)         
        self.training()
    ...
    def render_video(self, example_name=None, progress=gr.Progress()):
        
        videopath = os.path.join(self.save_dir, f'{example_name}_4.mp4')
        ...
        for view in iterable_render:
            ...
            framelist.append(
                np.round(frame.permute(1,2,0).detach().cpu().numpy()[::-1].clip(0,1)*255.).astype(np.uint8))
