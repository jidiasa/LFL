...
import sys
import os
sys.path.append("/home/ubuntu/Desktop/WonderJourney")
from simple_knn._C import distCUDA2
from models.GSRefiner.utils.general import inverse_sigmoid, get_expon_lr_func, build_rotation
from models.GSRefiner.utils.system import mkdir_p
from models.GSRefiner.utils.sh import RGB2SH
from models.GSRefiner.utils.graphics import BasicPointCloud
from models.GSRefiner.utils.general import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        scales[:,2] = scales[:,2].clamp(max=0.1)
    ...
    def densify_and_split(self, grads,color_grads, grad_threshold, scene_extent, N=2):
        ...
        if selected_pts_mask.sum() + n_init_points > self.max_all_points:
            ...
            if limited_num <= 0:
                ...
                ratio = (selected_pts_mask.sum() + n_init_points - self.max_all_points) / selected_pts_mask.sum()
                # breakpoint()
                indices = selected_pts_mask.nonzero().squeeze(-1)
                indices = indices[torch.randperm(indices.shape[0])[:int(ratio * selected_pts_mask.sum())]]
                selected_pts_mask = torch.zeros_like(selected_pts_mask)
